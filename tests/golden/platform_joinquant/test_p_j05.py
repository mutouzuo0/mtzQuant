# coding:utf-8
# @author      : 木头左
# @create_time        : 2026/08/16 14:05:00
# @update_time        : 2026/08/16 21:59:08
# @description : g05/g06 平台版（聚宽）：停牌 stale 估值/过期复牌 + 现金不足拒单

"""g05/g06 平台版（聚宽, D3）。手算依据与 native 一致:
- g05: day10 买 10,000 → day11 成交 100,100+佣金 10.01 → 现金 899,889.99;
  day20-22 停牌 stale 估值 NAV=0.99988999 连续; 停牌日挂单 expire。
...
def test_g05_jq_suspend_valuation_stale(gdriver) -> None:
    """停牌段估值沿用前收: day19/20/22/23 NAV 全 = 0.99988999。"""
    gdriver.add_data({CODE: _suspended_bars()})
    snap = run_joinquant_golden(gdriver, JQ_G05)
...
def test_g05_jq_suspend_order_expire(gdriver) -> None:
    """停牌日（day20）挂单 → expire + 降级。"""
    gdriver.add_data({CODE: _suspended_bars()})
    snap = run_joinquant_golden(gdriver, JQ_G05_EXPIRE)
...
def test_g06_jq_insufficient_cash_reject(gdriver) -> None:
    ...
    snap = run_joinquant_golden(gdriver, JQ_G06_BIG)
...
def handle_data(context, data):
    g.n += 1
    if g.n == 1:
        order_target_value('600000.XSHG', 49000)
"""


def test_g06_jq_within_budget_succeeds(gdriver) -> None:
    """对照: 目标 49,000 → 4,900 股成交（10.01×4,900=49,049）。"""
    gdriver.account.available_cash = 100_000.0
    gdriver.add_data({CODE: flat_series(CODE, 10)})
    snap = run_joinquant_golden(gdriver, JQ_G06_OK)
